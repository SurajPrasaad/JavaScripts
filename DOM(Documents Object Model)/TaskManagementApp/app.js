...
addBoardBtn.addEventListener("click", () => {
    ...
    createEditElement.addEventListener("click", () => {
        let updateBoardValue = prompt("Update board title", createElementH4.textContent);
        if (updateBoardValue) {
...
createTask.addEventListener("click", () => {
    ...
    let boardTitles = boards.map((board, index) => `${index + 1}: ${board.querySelector("h4").textContent}`).join("\n");
